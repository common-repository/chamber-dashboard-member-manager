   ...
   if ( array_key_exists( 'cdash-member-manager', $wp->query_vars ) && $wp->query_vars['cdash-member-manager'] == 'paypal-ipn' ) {
        if( !isset( $_POST['txn_id'] ) ) {
          ...
          header('HTTP/1.1 200 OK');
            ...
            // we don't already have an invoice with this transaction ID, so we'll process the request.
            ...
            //mail('chandrika@chamberdashboard.com', 'stuff', 'req: ' . print_r($req, true) . PHP_EOL. 'result: '.  print_r($result, true) . PHP_EOL . ' args:' . print_r( $pp_args, true )  . PHP_EOL . 'url: ' . $pp_url ) ;
            if( is_wp_error( $result ) || $result['response']['code'] != 200 ) {
              // HTTP ERROR
                if( is_wp_error( $result ) ){
                    ...
                    $error_message .= '<p>' . __( 'Please contact <a href="https://chamberdashboard.com/chamber-dashboard-support/">Chamber Dashboard support</a> for assistance.', 'cdashmm' ) . '</p>';
                ...
                if (strcmp( trim( $result['body'] ), "VERIFIED" ) == 0 ) {
                    //mail( "chandrika@chamberdashboard.com", "post data", "<pre>" . print_r($_POST, true) . "</pre>" );
                    // gather variables
                    ...
                    $today = date('Y-m-d');
                    //mail( "chandrika@chamberdashboard.com", "transaction id", $_POST['txn_id'] );
                    // get the invoice
                    $args = array(
                        'post_type' => 'invoice',
                    ...
                    if ( $thisinvoice->have_posts() ) :
                        while ( $thisinvoice->have_posts() ) : $thisinvoice->the_post();
                            //mail( "chandrika@chamberdashboard.com", "result of thisinvoice query", get_the_title() );
                            $invoice = get_the_id();
                            // if this is a subscription payment, check whether this invoice has already been paid
                            if( "subscr_payment" == $_POST['txn_type'] ) {
                                //mail( "chandrika@chamberdashboard.com", "this is a subscription payment", $_POST['txn_type'] );
                                if( has_term( 'paid', 'invoice_status' ) ) {
                                    // since this one has been paid, we need to create a duplicate of it
                                    //mail( "chandrika@chamberdashboard.com", "has been paid", "this invoice has paid status" );
                                    ...
                                    //mail( "chandrika@chamberdashboard.com", "new invoice id", $invoice );
                                    ...
                                    update_post_meta($invoice, '_cdashmm_item_tax', $invoice_meta['item_tax']);
                                    update_post_meta( $invoice, '_cdashmm_item_donation', $invoice_meta['item_donation'] );
                                    update_post_meta( $invoice, '_cdashmm_item_processing_fee', $invoice_meta['item_processing_fee'] );
                    ...
                    wp_reset_postdata();
                    //mail( "chandrika@chamberdashboard.com", "invoice id", $invoice );
                    ...
                    if( isset( $invoice ) ) {
                        ...
                        if( isset( $_POST['item_name1'] )  || 'subscr_payment' == $_POST['txn_type'] ) {
                            if( isset( $business ) ) {
                                ...
                                if( "subscr_payment" == $_POST['txn_type'] ) {
                                    ...
                                    $renewaldate = cdashmm_set_mrd();
                                    //cd_debug("Renewal Date: " . $renewaldate);
                        ...
                        $error_from = "Chamber Dashboard <" . $options['receipt_from_email'] . ">";
                        $error_to = $options['admin_email'];
                        $error_subject = __( 'Error Processing Payment', 'cdashmm' );
                        $error_message = '<p>' . __( 'Chamber Dashboard encountered an error processing a payment on your site.  We were unable to find the invoice associated with the transaction.  Please contact <a href="https://chamberdashboard.com/chamber-dashboard-support/">Chamber Dashboard Support</a>.', 'cdashmm' ) . '</p>';
                        $error_message .= '<p>' . __( 'Debug information: ', 'cdashmm' ) . '</p>';
                        $error_message .= '<pre>' . print_r($_POST, true) . '</pre>';
                        cdashmm_send_email( $error_from, $error_to, '', $error_subject, $error_message );
                    }
                  }
                elseif( strcmp( trim( $result ), "INVALID") == 0 ) {
                    $error_from = "Chamber Dashboard <" . $options['receipt_from_email'] . ">";
                    $error_to = $options['admin_email'];
                    $error_subject = __( 'Error Processing Payment', 'cdashmm' );
                    $error_message = '<p>' . __( 'Chamber Dashboard encountered an error processing a payment on your site.  Please contact <a href="https://chamberdashboard.com/chamber-dashboard-support/">Chamber Dashboard Support</a>.', 'cdashmm' ) . '</p>';
                    $error_message .= '<p>' . __( 'Debug information: ', 'cdashmm' ) . '</p>';
                    $error_message .= '<pre>' . print_r( $result , true ) . '</pre>';
                    cdashmm_send_email( $error_from, $error_to, '', $error_subject, $error_message );
                }
            }
        }
